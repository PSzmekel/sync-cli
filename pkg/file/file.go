...
func CopyFile(ctx context.Context, sourcePath, sourceFile, targetPath, targetFile string) error {
	...
	for i := 0; i < len(targetDir)-1; i++ {
		...
		if err != nil && stat == nil {
			err := exec.CommandContext(ctx, "mkdir", "-p", "--", dirPath).Run()
			if err != nil {
				return err
			}
		}
	}
	return exec.CommandContext(ctx, "cp", "-a", "--", filepath.Join(sourcePath, sourceFile), filepath.Join(targetPath, targetFile)).Run()
}
